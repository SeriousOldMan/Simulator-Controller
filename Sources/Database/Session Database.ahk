...
class EditLapsWindow extends Window {
	Close(*) {
		editLaps("Close")
	}
}
...
class SessionDatabaseEditor extends ConfigurationItem {
	...
	createGui(configuration) {
		...
		updateTelemetryNotes(*) {
			...
			if selected {
				...
				setMultiMapValue(info, "General", "Notes", encode(editor.Control["telemetryNotesEdit"].Text))
		...
		updateStrategyNotes(*) {
			...
			if selected {
				...
				setMultiMapValue(info, "General", "Notes", encode(editor.Control["strategyNotesEdit"].Text))
		...
		updateSetupNotes(*) {
			...
			if selected {
				...
				setMultiMapValue(info, "General", "Notes", encode(editor.Control["setupNotesEdit"].Text))
		...
		selectTrackPoint(*) {
			...
			if editor.findTrackCoordinate(x - editor.iTrackDisplayArea[1], y - editor.iTrackDisplayArea[2], &coordinateX, &coordinateY) {
				if (this.TrackEditorMode = "Automations") {
					...
					if action {
						if GetKeyState("Ctrl") {
							msgResult := withBlockedWindows(MsgDlg, translate("Do you really want to delete the selected action?")
																  , translate("Delete")
		...
		deleteTrackAutomation(*) {
			...
			msgResult := withBlockedWindows(MsgDlg, translate("Do you really want to delete the selected automation?")
												  , translate("Delete")
												  , {Options: 262436, Mode: "Question"
												   , Buttons: collect(["Yes", "No"], translate)})
		...
		chooseDatabaseScope(*) {
			...
			if GetKeyState("Ctrl") {
				msgResult := withBlockedWindows(MsgDlg, translate("Do you really want to change the scope for all applications?")
													  , translate("Modular Simulator Controller System")
													  , {Options: 262436, Mode: "Question"
													   , Buttons: collect(["Yes", "No"], translate)})
		...
		editorGui.Add("Button", "xp+235 yp+370 w23 h23 X:Move Y:Move vuploadTelemetryButton").OnEvent("Click", uploadTelemetry)
		...
		this.iStrategyListView := editorGui.Add("ListView", "x296 ys w360 h363 H:Grow X:Move(0.2) W:Grow(0.8) -Multi -LV0x10 AltSubmit NoSort NoSortHdr", collect(["Source", "Name", "Date"], translate))
		...
		editorGui.Add("Button", "xp+260 yp+370 w23 h23 X:Move Y:Move vuploadStrategyButton").OnEvent("Click", uploadStrategy)
		...
		this.iSetupListView := editorGui.Add("ListView", "x296 yp+24 w360 h339 H:Grow X:Move(0.2) W:Grow(0.8) -Multi -LV0x10 AltSubmit NoSort NoSortHdr", collect(["Source", "Name", "Date"], translate))
		...
		editorGui.Add("Button", "xp+260 yp+346 w23 h23 Y:Move X:Move vuploadSetupButton").OnEvent("Click", uploadSetup)
	...
	updateState() {
		...
		if selected {
			...
			if (type != translate("Community")) {
				...
				window["telemetryNotesEdit"].Enabled := true
				...
				window["telemetryNotesEdit"].Enabled := false
			}
		}
		else {
		...
		if selected {
			...
			if (type != translate("Community")) {
				...
				if (!info || (getMultiMapValue(info, "Origin", "Driver", false) = this.SessionDatabase.ID)) {
					...
					window["strategyNotesEdit"].Enabled := true
				}
				else {
					...
					window["strategyNotesEdit"].Enabled := false
				}
			}
			else {
	...
	loadCar(car, force := false) {
		...
		if (force || (car != this.SelectedCar)) {
			...
			if (car == true)
				...
				window["carDropDown"].Choose(inList(this.CarNames, this.getCarName(this.SelectedSimulator, car)) + 1)
	...
	loadTrack(track, force := false) {
		...
		if (force || (track != this.SelectedTrack)) {
			...
			if (track == true) {
				...
				window["trackDropDown"].Choose(inList(this.TrackNames, this.getTrackName(this.SelectedSimulator, track)) + 1)
	...
	loadStrategies(select := false) {
		local window := this.Window
		local userStrategies, communityStrategies, ignore, name, info, origin, date
		...
		for ignore, name in userStrategies {
			...
			if (select = name) {
				this.StrategyListView.Add("Select Vis", origin, name, date)

				select := this.StrategyListView.GetCount()
			}
			else
				this.StrategyListView.Add("", origin, name, date)
		}
	...
	loadSetups(setupType, force := false, select := false) {
		local window := this.Window
		local userSetups, communitySetups, ignore, name, info, origin, date
		...
		if (force || (setupType != this.SelectedSetupType)) {
			...
			for ignore, name in userSetups {
				...
				if (select = name) {
					this.SetupListView.Add("Select Vis", origin, name, date)

					select := this.SetupListView.GetCount()
				}
				else
					this.SetupListView.Add("", origin, name, date)
			}
	...
	selectTrack(editor?) {
		...
		if isSet(editor)
			if (editor = "Automations") {
				this.Control["trackEditorTypeDropDown"].Choose(2)

				this.iTrackEditorMode := "Automations"
			}
			else {
				this.Control["trackEditorTypeDropDown"].Choose(1)
	...
	selectModule(module, force := false, component?) {
		local window
	...
	selectTelemetry(row, open := false) {
		...
		window["telemetryNotesEdit"].Text := (info ? decode(getMultiMapValue(info, "General", "Notes", "")) : "")
	...
	selectStrategy(row, open := false) {
		...
		window["strategyNotesEdit"].Text := (info ? decode(getMultiMapValue(info, "General", "Notes", "")) : "")
	...
	selectSetup(row) {
		...
		window["setupNotesEdit"].Text := (info ? decode(getMultiMapValue(info, "General", "Notes", "")) : "")
	...
	deleteTelemetry(telemetryName) {
		...
		msgResult := withBlockedWindows(MsgDlg, translate("Do you really want to delete the selected lap telemetry?")
											  , translate("Delete")
											  , {Options: 262436, Mode: "Question"
											   , Buttons: collect(["Yes", "No"], translate)})
	...
	deleteStrategy(strategyName) {
		...
		msgResult := withBlockedWindows(MsgDlg, translate("Do you really want to delete the selected strategy?")
											  , translate("Delete")
											  , {Options: 262436, Mode: "Question"
											   , Buttons: collect(["Yes", "No"], translate)})
	...
	deleteSetup(setupType, setupName) {
		...
		msgResult := withBlockedWindows(MsgDlg, translate("Do you really want to delete the selected setup?")
											  , translate("Delete")
											  , {Options: 262436, Mode: "Question"
											   , Buttons: collect(["Yes", "No"], translate)})
...
editLaps(editorOrCommand, arguments*) {
	...
	if (editorOrCommand = "Close")
		result := kOk
	else if (editorOrCommand = "LoadElectronics") {
		electronicsListView.Delete()
	...
	else if (editorOrCommand = "LoadTyres") {
		tyresListView.Delete()
...
editSettings(editorOrCommand, arguments*) {
	...
	local availableServerURLs, settings, chosen, msgResult, consent
	...
	rebuildDatabase(*) {
		...
		msgResult := withBlockedWindows(MsgDlg, translate("Do you really want to rebuild the local database?")
											  , translate("Team Server")
											  , {Options: 262436, Mode: "Question"
											   , Buttons: collect(["Yes", "No"], translate)})
