...
| Strategy Update | [Required] strategy    | No | Yes | Loads the given strategy supplied in textual JSON format and informs the driver about a new available strtegy. This action is triggered by the "Strategy Recommendation" event and must be enabled together with this event. It is only meant to be called in this context. |
