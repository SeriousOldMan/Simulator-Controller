...
Some models, especially *smaller* ones, cannot follow all your instructions as required. For example, a model may insist to prepend a "Rephrased text: " before every text returned by the *Rephrasing* booster. This will be a big problem for the immersion, of course. In this case, you can filter and rewrite the answer given by the model using a [Lua](https://www.lua.org) script.

This script, which can be created or modified by clicking on the button labeled "Filter...", will have access to a single global variable named "Answer", which holds the text generated by the model. The script must return the new answer as its single value.
...
Example: For the *problem* mentioned above, when a model prepends a "Rephrased text: " for every text generated by the *Rephrasing* booster, the filter script will be as simple as:

	return string.gsub(Answer, "Rephrased text:", "")
