...
A word about memory consumption: Althoug a lot of effort has been put into optimizing the memory consumption of the overall solution, the Race Assistants still consume a lot of memory, which is especially to be considered during endurace races. You can calculate about 1GB of memory usage for a 1-hour stint, so take this into account for very long endurance races. A 32GB machine is recommend when participating in a 24h race, for example.
