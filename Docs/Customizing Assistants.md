...
But is also possible to connect the rule based Assistants to a LLM to improve the cnversational capabilities or to alter or even extend the behavior of them. This all can be achieved by configuring the so called Assistant Boosters. The following documentation will give you all information you need to setup the Assistant Boosters.
...
## Connecting an Assistant to a LLM
...
- As said, the available data depends on the current simulator. If available, information about tyre wear, brake temperatures and wear and so on are included as well.
...
Additionally, you can allow the LLM to *call* [predefined or custom actions](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Installation-&-Configuration#managing-actions) as a result of your conversation. For example, if you ask the Strategist whether an undercut might be possible in one of the next laps, the LLM may call the Monte Carlo traffic simulation using an internal action. Which actions will be available to the LLM depends on the current Assistant. Be sure, to check out the [examples](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Customizing-Assistants#examples) down below.

Please be aware that you are talking to an AI based on GPT technology when using the *Conversation* booster. And AIs can lie or can give wrong answers. More details on that can be found below in the [How it works section](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Customizing-Assistants#how-it-works). To indicate that the upcoming answer will be generated by a LLM using the the *Conversation* booster, a short sound is played before the answer is given. As with the *Push-to-Talk* sound, you can route this acknowledge sound to a different device in a streaming setup, if required.

IMPORTANT: When action handling is enabled, it might be necessary to disable the *Understanding* booster or at least set the "Creativity" to a very low value. Otherwise the *Understanding* booster might detect a command pattern, which will match to a pre-defined voice command, thereby preventing the LLM giving you an answer based on the supplied knowledge or even creating a custom action plan.
...
The *Reasoning* booster implements a LLM agent framework architecture also used by famous open-source agent development environments like [LangChain](https://github.com/langchain-ai/langchain) or [n8n)[https://github.com/n8n-io/n8n]. The difference here is the deep integration with the rule engine of the Assistants as the central instance for the LLM activation.

As already mentioned, you can choose a different LLM for this booster, because here strong reasoning skills are way more important than conversational excellence. Additionally, a LLM used for the *Reasoning* booster must support tool calling.
...
Some models, especially *smaller* ones, cannot follow all your instructions as required. For example, a model may insist to prepend a "New text: " before every text returned by the *Rephrasing* booster. This will be a big problem for the immersion, of course. In this case, you can filter and rewrite the answer given by the model using a [Lua](https://www.lua.org) script.

This script, which is specific for the selected GPT provider, can be created or modified by clicking on the button labeled "Filter...". It will have access to a single global variable named "Answer", which holds the text generated by the model. The script must return the new answer as its single value.

Good to know: During the execution of the script you can use the *extern* function to call any global *AutoHotkey* object in the current process. Example: extern("showMessage")("Hello World!"). Beside calling functions, *extern* can also be used to access any global variable in that process.

Example: For the *problem* mentioned above, when a model prepends a "New text: " for every text generated by the *Rephrasing* booster, the filter script will be as simple as:
...
4. An action can do almost anything. Typically it triggers a special behavior of the Assistant, for example, create a pitstop plan, but it can also call any of the available [action functions](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Action-Functions) in Simulator Controller, or it can generate voice output, and so on. Using *Assistant.Raise* (see below for [more information](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Customizing-Assistants#managing-events)), it can also raise additional events, even for other Assistants. These enables the LLM to create a complex chain of thought or collaborate with other Assistants.
...
1. When a LLM is activated, it is possible to provide a list of function descriptions to the LLM. *OpenAI* has defined the quasi-standard for the function description in JSON format as part of their API:
...
4. Equally useful as "Assistant Rule" is "Assistant Script" and since *Lua* is a widespread scripting language, it may be more accessible to the typical user. When defining the script for a custom action, you can use the following functions to connect to the given Assistant or even the "Simulator Controller.exe" process:
   ...
   - Rules.SetValue(fact :: \<string\>, value :: \<string\>)
     ...
     Changes the value for the given fact in the knowledge base of the Assistant.
...
|-----------------------------|-------------------|--------------|-----------|--------------------------|
| Pitstop Planning            | 1. [Optional] targetLap<br>2. [Optional] refuelAmount<br>3. [Optional] tyreCompounds<br>4. [Optional] repairDamage<br>5. [Optional] swapDriver | Yes | Yes | "We must pit for repairs. Can you create a plan without refueling and without tyre change?"<br>*repairDamage* and *swapDriver* indicate using a *Boolean* whether the repsective service will be provided during pitstop. A tyre compound for the new tyres can be supplied with *tyreCompound*, if a tyre change is requested. In case inidividual tyre change is possible, *tyreCompound* can be a list of tyre compounds seperated by ",". The number of supplied tyre compounds must be equal to the number of individually servicable wheels or axles in this case. If one of the tyres should not be changed, supply "-" or "false" for this specific wheel.<br>Example: "Dry (M), -, Dry (M), -" will change the tyres on both wheels on the left side. |
...
| Pitstop Recommendation | [Required] lap    | No | Yes | Informs the driver about a recommended lap for an upcoming pitstop. This action is triggered by the "Pitstop Recommendation" event and must be enabled together with this event. |
| Strategy Recommentation (1) | -                 | Yes | No | "Can you check whether we can skip the last pitstop, if we use a fuel saving map from now on?" |
| Strategy Recommendation (2) | [Optional] lap    | No | Yes | Recalculates the current strategy. If the optional lap number for the next pitstop is supplied, the Strategist will check whether a pitstop at a given lap might be beneficial and is also allowed for the currently active strategy. |
| Strategy Update | [Optional] strategy    | No | Yes | Loads the given strategy supplied in textual JSON format and informs the driver about a new available strtegy. This action is triggered by the "Strategy Recommendation" event and must be enabled together with this event. It is only meant to be called in this context and must be called even if the strategy is unchanged. In this case no argument for *strategy* must be passed to the call. |
...
| Pitstop Recommendation      | [Optional] targetLap  | This event, if enabled, will route the request to recommend an upcoming pitstop to the LLM together with all available data about the session and car state. The LLM acting as a LAM (aka large action model) in this case, may come up with a better lap for the upcoming pitstop than the internal rule engine. But this depends on the capabilities of the configured LLM, so be careful and use enable at your own risk. If the event is enabled, the corresponding action must be enabled as well.<br><br>If *targetLap* is supplied, it indicates a request by the driver to pit around this lap. |
| Strategy Simulation      | [Optional] pitstopLap  | This event, if enabled, will route the request to simulate a strategy and possibly update the current strategy or recommend a new one to the LLM together with all available data about the session and car state. The LLM acting as a LAM (aka large action model) in this case, may come up with a better strategy than the internal rule engine. But this depends on the capabilities of the configured LLM, so be careful and use enable at your own risk. If the event is enabled, the "Strategy Update" action must be enabled as well.<br><br>If *pitstopLap* is supplied, it indicates a request by the driver to pit around this lap. |
...
| Focused Gap Update          | 1. [Required] carNumber<br>2. [Required] delta<br>3. [Required] change | This event is signalled periodically, when the gap to the focused car has changed for a given amount, which can be configured in the "Session Database". *carNumber* is the car or race number of the car ahead and *delta* is the current gap in seconds. *change* is either "Gained" or "Lost", indicating whether the gap got larger or smaller. |
...
| Opponent Pitting            | [Optional] opponentPosition | Informs about an opponent going to the pits. If this is a direct opponent, *opponentPosition* can be supplied and must be one of "Ahead", "Behind" or "Focused". |
