...
[6.1.6.0-dev]() (Early build for 6.1.6. Changes: Remember choice of layout in Telemetry Viewer, Fixed missing mode flag in Startup Profiles, Fixed voice recognition test mode, Corners can be given names in track maps, Driving Coach understands and uses corner names, Prevent pitstop preparation if the car is in the pitlane, Optimized pitlane location detection, Show corner names in the title bar of the telemetry inspector, Remove meta information from thinking LLMs.)
