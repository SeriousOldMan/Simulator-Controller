...
class PedalCalibrationStepWizard extends ActionsStepWizard {
	...
	Pages {
		Get {
			...
			if (wizard.isModuleSelected("Controller") && wizard.isModuleSelected("Pedal Calibration") && !wizard.BasicSetup)
				return 1
			else
				return 0
		}
	...
	saveToConfiguration(configuration) {
		...
		Plugin("Pedal Calibration", false, wizard.isModuleSelected("Pedal Calibration"), "", arguments).saveToConfiguration(configuration, false)
	}
	...
	createGui(wizard, x, y, width, height) {
		...
		widget8 := window.Add("HTMLViewer", "x" . x . " yp+265 w" . width . " h80 Y:Move(0.66) W:Grow H:Grow(0.33) VpedalCalibrationInfoText Hidden")
	...
	savePage(page) {
		...
		if super.savePage(page) {
			if !this.SetupWizard.isSoftwareInstalled("SmartControl") {
				msgResult := withBlockedWindows(MsgDlg, translate("Heusinkveld SmartControl cannot be found. Do you really want to proceed?")
													  , translate("Warning")
