...
bool checkPositions() {
	...
	if ((velocityX != 0) || (velocityY != 0) || (velocityZ != 0)) {
		...
		*/
		...
		float coordinateZ = gf->car_coordinates[carID][1];
		float speed = 0.0;
		...
		for (int id = 0; id < min(gf->active_cars, 60); id++) {
			if (id != carID) {
				float otherSpeed = vectorLength(lastCoordinates[id][0] - gf->car_coordinates[id][0],
												lastCoordinates[id][2] - gf->car_coordinates[id][2]);

				if ((abs(speed - otherSpeed) / speed < 0.5) && sameHeading(lastCoordinates[carID][0] - coordinateX,
																		   lastCoordinates[carID][2] - coordinateY,
																		   lastCoordinates[id][0] - gf->car_coordinates[id][0],
																		   lastCoordinates[id][2] - gf->car_coordinates[id][2])) {
					bool faster = false;
					...
					newSituation |= checkCarPosition(coordinateX, coordinateY, coordinateZ, angle, faster,
						gf->car_coordinates[id][0], gf->car_coordinates[id][2], gf->car_coordinates[id][1]);
		...
		for (int id = 0; id < gf->active_cars; id++) {
			lastCoordinates[id][0] = gf->car_coordinates[id][0];
			lastCoordinates[id][1] = gf->car_coordinates[id][1];
			lastCoordinates[id][2] = gf->car_coordinates[id][2];
		}
...
void updateIdealLine(int carIdx, double running, double speed) {
	...
	idealLine[(int)std::round(running * (idealLine.size() - 1))].update(speed, gf->car_coordinates[carIdx][0], gf->car_coordinates[carIdx][2]);
}
...
bool getLastcar_coordinates(int carIndex, long milliSeconds, float* speed) {
	float lastX, lastY;

	if (getLastcar_coordinates(carIndex, &lastX, &lastY)) {
		SPageFileGraphicEvo* gf = (SPageFileGraphicEvo*)m_graphics.mapFileBuffer;

		float newPosX = gf->car_coordinates[carIndex][0];
		float newPosY = gf->car_coordinates[carIndex][2];
...
void updateLastcar_coordinates(bool init) {
	lastcar_coordinatesUpdate = GetTickCount();

	if (init) {
		lastcar_coordinates.clear();
		lastcar_coordinatesCount = 0;
	}
	else {
		SPageFileGraphicEvo* gf = (SPageFileGraphicEvo*)m_graphics.mapFileBuffer;
		...
		for (int i = 0; i < gf->active_cars; i++) {
			...
			if (lastcar_coordinates.contains(carID)) {
				...
				lastcar_coordinates[carID].posX = gf->car_coordinates[i][0];
				lastcar_coordinates[carID].posY = gf->car_coordinates[i][2];
			}
			else
				lastcar_coordinates[carID] = CarPosition(lastcar_coordinatesCount, gf->car_coordinates[i][0], gf->car_coordinates[i][2]);
		}
	}
}

inline bool hasValidcar_coordinates(long* milliSeconds) {
	(*milliSeconds) = GetTickCount() - lastcar_coordinatesUpdate;
	
	if ((*milliSeconds) < 75)
		return false;
	else if ((*milliSeconds) > 200) {
		updateLastcar_coordinates(false);
...
void updateTrackSpline() {
	try {
		if (trackSplineBuilding) {
			...
			float newPosX = gf->car_coordinates[referenceDriverIdx][0];
			float newPosY = gf->car_coordinates[referenceDriverIdx][2];
			...
			*/
			...
			if (distance > 0) {
				if (done) {
					...
					if (!trackSplineReady || trackSplineRebuild || ((gf->last_laptime_ms > 0) && ((gf->last_laptime_ms * 1.002) < bestLapTime))) {
						bestLapTime = gf->last_laptime_ms;
						...
						updateLastcar_coordinates(true);
...
bool startTrackSplineBuilder(int driverIdx, bool rebuild = false) {
	...
	if (baseLap == -1)
		baseLap = gf->total_lap_count + 1;
	else if (baseLap <= gf->total_lap_count) {
		...
		referenceDriverPosX = gf->car_coordinates[driverIdx][0];
		referenceDriverPosY = gf->car_coordinates[driverIdx][2];
...
float getRunning(int carIdx) {
	...
	string key = std::to_string((long)round(gf->car_coordinates[carIdx][0] / 5)) + "|" +
				 std::to_string((long)round(gf->car_coordinates[carIdx][2] / 5));
...
inline float getSpeed(int carIdx, long milliSeconds) {
	...
	if (getLastcar_coordinates(carIdx, milliSeconds, &speed))
		return speed;
	else
		return -1;
}
...
bool checkAccident() {
	...
	if (gf->is_in_pit_box || gf->is_in_pit_lane) {
		trackSplineBuilding = false;
	...
	if (gf->total_lap_count > completedLaps) {
		...
		completedLaps = gf->total_lap_count;
		numAccidents = 0;
	}
	...
	*/
	...
	if (!hasValidcar_coordinates(&milliSeconds))
		return false;
...
bool checkFlagState() {
	...
	if (gf->global_flag & AC_YELLOW_FLAG) {
	...
	else if ((lastFlagState & YELLOW_ALL) != 0) {
...
bool greenFlag() {
	...
	if (!greenFlagReported && (gf->flag & AC_GREEN_FLAG) && (getSession(sf->session, gf->session_state.phase_name) == "Race")) {
...
void updateTopSpeed() {
	...
	if (gf->total_lap_count > lastLaps) {
...
bool writeCoordinates() {
	...
	if (!mapStarted)
		if (mapLap == -1) {
			mapLap = gf->total_lap_count;
		
			return true;
		}
		else if (gf->total_lap_count == mapLap)
			return true;
	...
	if (trackSplineReady) {
		...
		if (driverRunning > 0)
			try {
				...
				if ((gf->total_lap_count + 1) != telemetryLap) {
					try {
						telemetryFile.close();
					...
					telemetryLap = (gf->total_lap_count + 1);
...
inline const bool active() {
	if (started)
		...
		if ((getSession(sf->session, gf->session_state.phase_name) == "Race") && (gf->flag != AC_GREEN_FLAG) && (gf->total_lap_count == 0))
			return false;
	}
...
int main(int argc, char* argv[])
{
	...
	if (startTelemetryLap == -1)
		startTelemetryLap = gf->total_lap_count + 1;
		...
		if (mapTrack) {
			...
			if (running) {
				if (carTelemetry)
					...
					if ((gf->status == AC_LIVE) && !gf->is_in_pit_box && !gf->is_in_pit_lane) {
						updateTopSpeed();
