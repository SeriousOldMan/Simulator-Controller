...
As an alternative to the Azure cloud services an integration with the voice recognition services of Google is also available. Google also provides a free contingent per month and the recognition quality is very similar.

Please note that it might be necessary to use one of the cloud based voice recognition services must be used, if you want to talk to Aiden, the Virtual Driving Coach. This might become necessary, since the interaction with Aiden does not use pattern based commands, for which the builtin voice recognition engines of Windows are optimized for. The cloud-based voice recognition services are way better, when it comes to free speech.
