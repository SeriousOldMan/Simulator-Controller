## 5.6.0.0-release 02/23/24 (planned)
  ...
  7. There is a new *Preferences* tab in the "Strategy Workbench", which allows you to set preferences for one or more selected pitstops. This will fix the choices for lap, refuel and tyre changes for this particular pitstop. But there are some drawbacks, so read the [new documentation](*) carefully.
  8. The default for the automatic pressure loss correction is now *Off* in the "Race Center".
