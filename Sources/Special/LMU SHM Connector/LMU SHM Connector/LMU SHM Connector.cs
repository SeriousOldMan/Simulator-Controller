...
using static SHMConnector.LeMansUltimateConstants.LMUGamePhase;
using static SHMConnector.LeMansUltimateConstants.LMUPitState;
...
namespace SHMConnector {
	public class SHMConnector {
		...
		// Read buffers:
		MappedBuffer<LMUTelemetry> telemetryBuffer = new MappedBuffer<LMUTelemetry>(LeMansUltimateConstants.MM_TELEMETRY_FILE_NAME, true /*partial*/, true /*skipUnchanged*/);
		MappedBuffer<LMUScoring> scoringBuffer = new MappedBuffer<LMUScoring>(LeMansUltimateConstants.MM_SCORING_FILE_NAME, true /*partial*/, true /*skipUnchanged*/);
		MappedBuffer<LMURules> rulesBuffer = new MappedBuffer<LMURules>(LeMansUltimateConstants.MM_RULES_FILE_NAME, true /*partial*/, true /*skipUnchanged*/);
		MappedBuffer<LMUForceFeedback> forceFeedbackBuffer = new MappedBuffer<LMUForceFeedback>(LeMansUltimateConstants.MM_FORCE_FEEDBACK_FILE_NAME, false /*partial*/, false /*skipUnchanged*/);
		MappedBuffer<LMUGraphics> graphicsBuffer = new MappedBuffer<LMUGraphics>(LeMansUltimateConstants.MM_GRAPHICS_FILE_NAME, false /*partial*/, false /*skipUnchanged*/);
		MappedBuffer<LMUPitInfo> pitInfoBuffer = new MappedBuffer<LMUPitInfo>(LeMansUltimateConstants.MM_PITINFO_FILE_NAME, false /*partial*/, true /*skipUnchanged*/);
		MappedBuffer<LMUWeather> weatherBuffer = new MappedBuffer<LMUWeather>(LeMansUltimateConstants.MM_WEATHER_FILE_NAME, false /*partial*/, true /*skipUnchanged*/);
		MappedBuffer<LMUExtended> extendedBuffer = new MappedBuffer<LMUExtended>(LeMansUltimateConstants.MM_EXTENDED_FILE_NAME, false /*partial*/, true /*skipUnchanged*/);

		// Write buffers:
		MappedBuffer<LMUHWControl> hwControlBuffer = new MappedBuffer<LMUHWControl>(LeMansUltimateConstants.MM_HWCONTROL_FILE_NAME);
		MappedBuffer<LMUWeatherControl> weatherControlBuffer = new MappedBuffer<LMUWeatherControl>(LeMansUltimateConstants.MM_WEATHER_CONTROL_FILE_NAME);
		MappedBuffer<LMURulesControl> rulesControlBuffer = new MappedBuffer<LMURulesControl>(LeMansUltimateConstants.MM_RULES_CONTROL_FILE_NAME);
		MappedBuffer<LMUPluginControl> pluginControlBuffer = new MappedBuffer<LMUPluginControl>(LeMansUltimateConstants.MM_PLUGIN_CONTROL_FILE_NAME);

		// Marshalled views:
		LMUTelemetry telemetry;
		LMUScoring scoring;
		LMURules rules;
		LMUForceFeedback forceFeedback;
		LMUGraphics graphics;
		LMUPitInfo pitInfo;
		LMUWeather weather;
		LMUExtended extended;

		// Marashalled output views:
		LMUHWControl hwControl;
		LMUWeatherControl weatherControl;
		LMURulesControl rulesControl;
		LMUPluginControl pluginControl;
		...
		public string ReadStandings()
		{
			...
			if (connected)
			{
                ...
                {
                    ref LMUVehicleScoring vehicle = ref scoring.mVehicles[i - 1];
					...
					*/
					...
					ref LMUVehicleTelemetry telemetry = ref GetPlayerTelemetry(vehicle.mID, ref this.telemetry);
		...
		public void LogToTempFile(string message)
		{
			try
			{
				...
				string logFile = Path.Combine(tempPath, "LMUSHMConnector.log");
        ...
        private void Connect() {
			if (!this.connected) {
				try {
					...
					this.hwControl.mLayoutVersion = LeMansUltimateConstants.MM_HWCONTROL_LAYOUT_VERSION;

					this.weatherControlBuffer.Connect();
					this.weatherControlBuffer.GetMappedData(ref this.weatherControl);
					this.weatherControl.mLayoutVersion = LeMansUltimateConstants.MM_WEATHER_CONTROL_LAYOUT_VERSION;

					this.rulesControlBuffer.Connect();
					this.rulesControlBuffer.GetMappedData(ref this.rulesControl);
					this.rulesControl.mLayoutVersion = LeMansUltimateConstants.MM_RULES_CONTROL_LAYOUT_VERSION;

					this.pluginControlBuffer.Connect();
					this.pluginControlBuffer.GetMappedData(ref this.pluginControl);
					this.pluginControl.mLayoutVersion = LeMansUltimateConstants.MM_PLUGIN_CONTROL_LAYOUT_VERSION;
		...
		private void SendPitMenuCmd(byte[] commandStr, double fRetVal) {
			if (commandStr != null) {
				...
				this.hwControl.mControlName = new byte[LeMansUltimateConstants.MAX_HWCONTROL_NAME_LEN];
