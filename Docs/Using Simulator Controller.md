...
|                  | Motivation Coaching                   | Controller Action | If enabled, telemetry coaching by the Driving Coach will be automatically enabled. This also includes automatic enabling of the motivation shout outs of the Driving Coach, once telemetry data is available. This coaching mode is most useful during races. If you want to have more control over the process, use the voice commands or take a look [here](https://github.com/SeriousOldMan/Simulator-Controller/wiki/AI-Driving-Coach#automatic-activation-of-coaching-mode). |
...
| Actions         | Covers the controller action functions ["speak" and "play"](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Action-Functions#trigger-actions), which can be used from scripts or in track automations and so on. |
...
### Data Latency and Other Real-Time Issues

The processing chain, from the request for data from the simulation to the spoken information delivered by one of the Assistants, takes some time. The more steps involved, the longer this can take. The most important basic setting is "Data: Update Frequency" in the [settings](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Session-Settings) section of the "Session Database." This value determines how frequently the data is queried and processed by the simulation. High-performance PCs can handle values ​​of 1-2 seconds for this setting. However, the default value is 10 seconds to prevent mid-range PCs from being overwhelmed by the comprehensive AI processing. Beside that, all Assistants have additional settings in their basic configuration and also in the "Session Database" that determine how they react to events and their timing (see especially [here](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Installation-&-Configuration#tab-race-spotter), [here](https://github.com/SeriousOldMan/Simulator-Controller/wiki/AI-Race-Spotter#opponent-and-delta-information) and [here](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Session-Settings)). Choose these settings, so that the corresponding information is given early enough. Speech generation and recognition, as well as additional processing steps by a connected LLM (aka Large Language Model), have a significant impact as well. For speech generation, the methods integrated directly into Windows naturally perform best, followed by Azure and Google, provided a nearby data center is used. If a LLM is to be integrated into the processing, it is important to ensure that it has low latency. Smaller models, in particular, offer significant advantages in this regard, but are also less "intelligent." In summary, with poorly chosen settings, overall latency can easily exceed 10 seconds, which can be disruptive when displaying time intervals or other information directly related to the situation on the track.
