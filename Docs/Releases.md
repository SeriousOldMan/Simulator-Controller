...
#### Changes
  ...
  - [Experts] The local LLM Runtime, which can be installed from a downloadable preset in "Simulator Setup", has been completely rewritten.
    - It now supports tool/function calling, which is required to run actions in the *Conversation* booster or even use it for the *Reasoning* booster. The LLM Runtime therefore now provides the same functionality as running a LLM in Ollama locally, but with a much better resource footprint and even some minor performance improvements. See the [updated documentation](https://github.com/SeriousOldMan/Simulator-Controller/wiki/AI-Driving-Coach#llm-runtime) for more information.
	  ...
	  A good model for successful experiments with local model execution incl. tool calling (actions) might be Qwen 3 with 27B parameters in minimum Q4_K_M quantization (Q6_K or higher will be much better, of course, if your system can handle it). Can be found on [Hugging Face](https://huggingface.co/bottlecapai/ThinkingCap-Qwen3.6-27B), for example. Requires at least a machine with 64 GB Ram and 16 GB VRam. Even then the model needs to be run partially on CPU.
