...
class RaceEngineer extends ConfigurationItem {
	...
	createRace(data) {
		...
		facts := {"Race.Car": getConfigurationValue(data, "Race Data", "Car", "")
				...
				, "Race.Setup.Tyre.Compound": getConfigurationValue(data, "Car Data", "TyreCompound", getConfigurationValue(settings, "Race Setup", "Tyre.Compound", "Dry"))
	...
	updateLap(lapNumber, data) {
		...
		for index, position in ["Front", "Rear", "Left", "Right", "Center"] {
			...
			if (Round(knowledgeBase.getValue(fact, 0)) < Round(newValue)) {
				knowledgeBase.setValue(fact, newValue)
