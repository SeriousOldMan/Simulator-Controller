...
## 6.2.3.0-release 03/14/25 (planned)
  ...
  7. If telemetry files are imported from MoTeC, the track is now scanned for coordinates, so that the telemetry data can be used by the Drving Coach. Therefore, frm now on importing of MoTeC files is only possible, if a track map is available for the given track selection.
