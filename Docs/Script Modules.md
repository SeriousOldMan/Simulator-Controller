...
| Data          | This property gives you access to the internal data represenation of the current session state (as returned by *Simulator.Read* (without pitstop setup information). It is a string in "INI" format as [described in the Assistants documentation](https://github.com/SeriousOldMan/Simulator-Controller/wiki/AI-Race-Engineer#telemetry-integration). Using this property is almost identical to calling the *Read* function of the "Simulator" module, but since this information is processed asynchronously, it may be a few seconds *old*. |
...
![](https://github.com/SeriousOldMan/Simulator-Controller/blob/main/Docs/Images/Simulator%20Module%20Example%201.jpg)
	...
	local function currentBrakeBalance()
		local ok, ini = pcall(Simulator.Read, "Le Mans Ultimate", "Session")
