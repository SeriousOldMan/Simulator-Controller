...
| call | target, method, [Optional] argument, ... | System | Invokes an internal method. *target* maybe either "Controller" (or "Simulator Controller") for method of the single controller instance itself or the name of a registered plugin and *method* is the name of the method to invoke for this target. You can supply any number of arguments to the call. |
