...
class Strategy extends ConfigurationItem {
	...
	class Pitstop extends ConfigurationItem {
		...
		initialize(tyreCompound, tyreCompoundColor, adjustments := false) {
			...
			local lastStintLaps := Floor(Min(remainingFuel / fuelConsumption, strategy.LastPitstop ? (lap - strategy.LastPitstop.Lap)
																								   : ((strategy.StartLap = 0) ? lap
																															  : (lap - strategy.StartLap))))
		...
		getDescriptor() {
			local tyreChange := kFalse
			local tyreCompound, nr

			if this.iTyreChange {
				tyreCompound := compound(this.TyreCompound, this.TyreCompoundColor)
				tyreChange := {TyreCompound: tyreCompound}
				...
				loop {
					...
					if this.Strategy.Pitstops[nr].TyreCompound {
						tyreCompound := compound(this.Strategy.Pitstops[nr].TyreCompound
											   , this.Strategy.Pitstops[nr].TyreCompoundColor)
	...
	saveToConfiguration(configuration) {
		...
		setMultiMapValue(configuration, "Setup", "TyreSet", this.TyreSet)
	...
	getDescriptor() {
		local tyreSets := []
		local pitstops := []
		local ignore, tyreSet, nr, fixedPitstop, fixedPitstops, fuelService
	...
	createStints(currentStint, currentLap, currentStintTime, currentSessionTime
		...
		local sessionLaps, numPitstops, fuelLaps, canonicalStintLaps, remainingFuel, tyreSetConsumed, maxLaps, overLaps
		...
		if !currentTyreSet
			currentTyreSet := this.availableTyreSet(this.TyreCompound, this.TyreCompoundColor
												  , this.tyreCompoundLife(this.TyreCompound, this.TyreCompoundColor)
												  , &ignore := false, false)
		...
		loop {
			...
			if (adjustments && adjustments.Has(pitstopNr) && adjustments[pitstopNr].HasProp("Lap"))
				...
				maxLaps := this.tyreCompoundLife(this.TyreCompound[true], this.TyreCompoundColor[true])
				overLaps := (maxLaps * this.TyreLapsVariation / 100 * (Min(100 - Sqrt(Random(0, 10000)), 100) / 100))

				if (this.RemainingTyreLaps[true] + overLaps - (pitstopLap - lastPitstopLap)
				  < Min(maxLaps, this.getMaxFuelLaps(this.FuelCapacity, this.FuelConsumption[true]))) {
					tyreSetConsumed := true

					this.consumeTyreSet(this.TyreCompound[true], this.TyreCompoundColor[true], this.TyreSet[true], (pitstopLap - lastPitstopLap))
				}
			}
			...
			if (lastPitstop && !tyreSetConsumed)
			...
			*/
			...
			if ((numPitstops && (pitstopNr <= numPitstops)) || ((pitstop.StintLaps > 0) && ((pitstop.RefuelAmount > 0) || pitstop.TyreChange))) {
				...
				if ((pitstopNr = 1) && !tyreSetConsumed)
