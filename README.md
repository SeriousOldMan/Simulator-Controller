...
[5.9.4.0-release](https://cutt.ly/JeSnLoko) (Changes: Layout settings for telemetry viewer, Fixed standings with duplicate race numbers, Fixed grid size and positions for R3E, Moved Spotter lap time announcement to Opponent Information group, Horizontal shifting of telemetry graphs, Save selected zoom factor for telemetry graphs, Independent horizontal and vertical zooming for telemetry graphs, Lap telemetry importer for MoTeC, Multi upload in session database, Upload telemetry can import from MoTeC and alike, New steer lock setting in "Session Database", Fixed occasional unwanted focused car messages in R3E, Optimized Spotter proximity alerts.)
...
[5.9.3.0-release](https://cutt.ly/KeAwuUcY) (Changes: Updated car meta data for R3E, Fixed selection of start stint pressures in "Team Center", Fixed an error in strategy simulation which did not respect the stint timer, Fixed saving of imported telemetry data, Spotter now compares lap time to that of the opponents, Optimized window resizing for "Session Database", Fixed configuration of booster actions, Fixed configuration of Reasoning Booster in "Simulator Setup", Weather-specific best lap times are now passed to the LLM of the Reasoning booster to work with, New race settings for ideal tyre temperature.)

[5.9.2.2-release](https://cutt.ly/PeOZv6e0) (Changes: Lap telemetry opened from "Session Database" show the driver name, New telemetry fields LatG, LongG and Curvature, Support to import telemetry data from "Second Monitor", Fixed iRacing Spotter, Multiple telemetry selection support when loading telemetries, Colorized Track Automation state display on Button Box, Telemetry Viewer usable in "Setup Workbench", Renamed "Telemetry Analyzer" to "Issue Analyzer", Renamed "Telemetry Browser" to "Telemetry Viewer", Track map support in Telemetry Viewer, Select position on track by clicking in telemetry graph, Telemetry Viewer shows rotating ball when collecting telemetry, Startup simulator option moved to Startup Profiles.)

#### Latest development build

[5.9.5.0-alpha](https://www.dropbox.com/scl/fi/lrct7kaigh7tz7lzauogu/Simulator-Controller-5.9.5.0-alpha.zip?rlkey=zkojxnm8r3v76xln79tawmgu1&st=0k84qder&dl=1) (Test build for 5.9.5. Changes: Several grammar fixes, Fixed RF2 telemetry file format, Section editor for track maps, New telemetry channel "Time", Telemetry smoothing, Telemetry support in "Setup Workbench" without specific car and track.)
