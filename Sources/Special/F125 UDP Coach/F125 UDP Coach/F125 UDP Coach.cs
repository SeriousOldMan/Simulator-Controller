...
namespace F125UDPCoach {
    public class UDPCoach
    {
        ...
        double vehicleSpeed(PacketMotionData motion, int carIndex)
        {
            var cm = motion.CarMotion[carIndex];
        ...
        // F1 car wheelbase approximately 3.6 metres
        const double F1WheelbaseMeter = 3.6;

        List<float> recentFrontWheelAngles = new List<float>();
        ...
        bool collectTelemetry(PacketMotionData motion, PacketMotionExData motionEx,
                              PacketLapData lapData, int playerIdx,
                              string soundsDirectory, string audioDevice)
        {
            if (motion == null || motionEx == null || lapData == null)
                return true;
            ...
            // F1 25 provides the front wheel angle directly in radians (MotionExData)
            float frontWheelAngle = smoothValue(recentFrontWheelAngles, motionEx.FrontWheelsAngle);

            float speed = (float)vehicleSpeed(motion, playerIdx);
            ...
            // F1 25 provides real angular velocity around Y axis (yaw) directly
            double angularVelocity = smoothValue(recentRealAngVels, motionEx.AngularVelocityY);

            // Compute ideal angular velocity using bicycle model:
            //   idealAngVel = v / R = v * frontWheelAngle / wheelbase  (small-angle approx)
            // Full computation via radius/perimeter for consistency with PMR approach:
            double steerAngleRadians = (double)frontWheelAngle;
            double radius = F1WheelbaseMeter / steerAngleRadians;
            ...
            if (Math.Abs(frontWheelAngle) > 0.01 && lastSpeed > 60)
            {
                ...
                int completedLaps = Math.Max(0, playerLap.CurrentLapNum - 1);
                CornerDynamics cd = new CornerDynamics(speed, 0, completedLaps, phase);

                if (Math.Abs(angularVelocity * 57.2958) > 0.1)
                {
                    double slip = Math.Abs(idealAngularVelocity - angularVelocity);

                    // Sign logic: FrontWheelsAngle < 0 = right turn (negative)
                    //   In a right turn, expected angularVelocity < 0 (clockwise)
                    //   If angularVelocity > 0 → car rotating opposite → oversteer
                    //   If angularVelocity < idealAngVel → more rotation than ideal → oversteer
                    // FrontWheelsAngle > 0 = left turn (positive)
                    //   In a left turn, expected angularVelocity > 0 (counter-clockwise)
                    //   If angularVelocity < 0 → car rotating opposite → oversteer
                    //   If angularVelocity > idealAngVel → more rotation than ideal → oversteer
                    ...
                    if (slip != 0)
                    {
                        ...
                        if (false)
                        {
                            ...
                            output.Write(frontWheelAngle + "  ");
                            output.Write((frontWheelAngle * 57.2958) + "  ");
        ...
        void checkCoordinates(PacketMotionData motion, PacketLapData lapData, int playerIdx)
        {
            if (DateTimeOffset.Now.ToUnixTimeMilliseconds() > nextUpdate)
            {
                var cm = motion.CarMotion[playerIdx];

                double velocityX = cm.WorldVelocityX;
                double velocityY = cm.WorldVelocityY;
                double velocityZ = cm.WorldVelocityZ;

                if ((velocityX != 0) || (velocityY != 0) || (velocityZ != 0))
                {
                    // F1 25: X/Z form the horizontal plane
                    double coordinateX = cm.WorldPositionX;
                    double coordinateY = cm.WorldPositionZ;
                    ...
                    if (triggerType == "Trigger") {
                        ...
                        if (lastLap != completedLaps)
                        {
                            lastLap = completedLaps;
        ...
        public void initializeAnalyzer(bool calibrateTelemetry, string[] args, int index)
        {
            ...
            if (calibrate) {
                // F1 25 uses FrontWheelsAngle directly — no steerLock/steerRatio needed.
                // Calibrate mode only needs lowspeedThreshold.
        ...
        public void Run(bool positionTrigger, bool trackHints, bool handlingAnalyzer) {
            ...
            while (true) {
                ...
                {
                    if (active()) {
                        byte playerIdx = receiver.GetPlayerCarIndex();
                        ...
                        if (positionTrigger)
                        {
                            if (motion != null && lapData != null)
                                checkCoordinates(motion, lapData, playerIdx);
