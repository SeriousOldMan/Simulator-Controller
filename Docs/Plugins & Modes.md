...
Simulator Controller must *understand* the current choices and the available settings in the Pitstop MFD, before any changes can be made automatically. There are two different methods available for this task:
  ...
  It is possible in very rare situations ore when you interfere using the keyboard yourself, while the option walk is running, that the search for the current pitstop settings get stucked in an endless loop. Press and hold Control together with Alt and Shift in this case, which will restart the process.
