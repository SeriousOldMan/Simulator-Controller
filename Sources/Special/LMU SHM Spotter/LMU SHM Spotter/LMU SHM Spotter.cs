...
using static LMUSHMSpotter.LeMansUltimateConstants;
using static LMUSHMSpotter.LeMansUltimateConstants.LMUGamePhase;
using static LMUSHMSpotter.LeMansUltimateConstants.LMUPitState;
using static System.Net.WebRequestMethods;
using static System.Windows.Forms.VisualStyles.VisualStyleElement.TextBox;

namespace LMUSHMSpotter {
	public class SHMSpotter {
		...
		bool checkPositions(ref LMUVehicleScoring playerScoring)
		{
        ...
        double vehicleSpeed(ref LMUVehicleTelemetry vehicle)
        {
            LMUVec3 localVel = vehicle.mLocalVel;
		...
		void updateIdealLine(ref LMUVehicleScoring vehicle, double running, double speed) {
        ...
        bool checkAccident(ref LMUVehicleScoring playerScoring)
        {
			bool accident = false;
        ...
        bool checkFlagState(ref LMUVehicleScoring playerScoring)
		{
			...
			if (playerScoring.mFlag == (byte)LMUPrimaryFlag.Blue)
			{
			...
			/*
			if (scoring.mScoringInfo.mGamePhase == (byte)LMUGamePhase.FullCourseYellow)
			{
		...
		bool checkPitWindow(ref LMUVehicleScoring playerScoring)
		{
		...
		bool greenFlag() {
			if (!greenFlagReported && (scoring.mScoringInfo.mGamePhase == (byte)LMUGamePhase.GreenFlag)
        ...
        void updateTopSpeed(ref LMUVehicleScoring playerScoring)
        {
		...
		bool writeCoordinates(ref LMUVehicleScoring playerScoring)
		{
			double lVelocityX = playerScoring.mLocalVel.x;
			double lVelocityY = playerScoring.mLocalVel.y;
			double lVelocityZ = playerScoring.mLocalVel.z;
		...
		void checkCoordinates(ref LMUVehicleScoring playerScoring)
		{
		...
		StringWriter telemetryData = null;
        ...
        void collectCarTelemetry(ref LMUVehicleScoring playerScoring)
        {
            ...
            try
            {
                if ((playerScoring.mTotalLaps + 1) != telemetryLap)
                {
                    try
                    {
                        if (telemetryFile != null) {
							telemetryFile.Write(telemetryData);
                            telemetryFile.Close();
                    ...
                    telemetryData = new StringWriter();
				...
				if (playerScoring.mLapDist > lastRunning)
                {
					...
					telemetryData.Write(playerScoring.mLapDist + ";");
					telemetryData.Write((float)vehicle.mFilteredThrottle + ";");
					telemetryData.Write((float)vehicle.mFilteredBrake + ";");
					telemetryData.Write((float)vehicle.mFilteredSteering + ";");
					telemetryData.Write((float)vehicle.mGear + ";");
					telemetryData.Write((float)vehicle.mEngineRPM + ";");
					telemetryData.Write(vehicleSpeed(ref playerScoring) + ";");

					telemetryData.Write(vehicle.mTCActive + ";");
					telemetryData.Write(vehicle.mABSActive + ";");

					telemetryData.Write((-playerScoring.mLocalAccel.z / 9.807f) + ";");
					telemetryData.Write((playerScoring.mLocalAccel.x / 9.807f) + ";");

					telemetryData.Write(playerScoring.mPos.x + ";");
					telemetryData.Write(-playerScoring.mPos.z + ";");

					telemetryData.Write(((vehicle.mElapsedTime - vehicle.mLapStartET) * 1000) + ";");

                    telemetryData.Write(vehicle.mLocalRot.z + ";");

                    telemetryData.Write(vehicle.mWheels[0].mSuspensionDeflection + ";");
                    telemetryData.Write(vehicle.mWheels[1].mSuspensionDeflection + ";");
                    telemetryData.Write(vehicle.mWheels[2].mSuspensionDeflection + ";");
                    telemetryData.WriteLine(vehicle.mWheels[3].mSuspensionDeflection);
                    ...
                    if (System.IO.File.Exists(telemetryDirectory + "\\Telemetry.cmd"))
                        try
                        {
                            ...
                            file.Write(vehicle.mTCActive + ";");
                            file.Write(vehicle.mABSActive + ";");
        ...
        public bool active() {
			if (started)
				return true;
						...
						&& (scoring.mScoringInfo.mGamePhase != (byte)LMUGamePhase.GreenFlag) && (GetPlayerScoring(ref scoring).mTotalLaps == 0))
				return false;
            ...
            while (true) {
				...
				if (connected)
				{
					...
					if (connected) {
						...
						{
							bool startGo = (scoring.mScoringInfo.mGamePhase == (byte)LMUGamePhase.GreenFlag);
