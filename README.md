...
[4.0.6-beta](https://www.dropbox.com/s/rpaeqzxa8w7t970/Simulator%20Controller%204.0.6-beta.zip?dl=1) (Test version for 4.0.6. New features: Fixed settings initialization in "Strategy Workbench", Fixed some pitstop search pictures for R3E, Optimized chart drawing, Validation methods for all float fields, Extensive logging for Team Server and "Race Center", Showing lap times in "M:SS.s" format, Display initial cold pressures in "Race Center" after first lap, Increased robustness of ACC Pitstop fallback algorithm, Improved thread management in "Race Center". Stable, incl. documentation and update procedure.)
