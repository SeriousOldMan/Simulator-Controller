...
- Finally, telemetry data can be imported from any tool, as long as an export in generic CSV format is supported. The following columns are supported:
  ...
  | SUSPDEFL_FL    | No            | The used travel of the front left shock absorber in mm. |
  | SUSPDEFL_FR    | No            | The used travel of the front right shock absorber in mm. |
  | SUSPDEFL_RL    | No            | The used travel of the rear left shock absorber in mm. |
  | SUSPDEFL_RR    | No            | The used travel of the rear right shock absorber in mm. |
...
5. You can also place strategies, lap telemetries and other stuff in the export folder, but this is not a typical use case for *foreign* data.
6. At the root of the export folder, you must create a "Export.info" file with the meta data. Here is an example:
...
7. Then ZIP the contents of the temporary folder and replace the file name extension "zip" with "export".
