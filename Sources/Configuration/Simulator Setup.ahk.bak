		...
		loop count
			if this.Steps.Has(A_Index) {
				...
				if step {
					...
					if FileExist(kUserHomeDirectory . "Setup\Definitions\" . step.Step . " Step.ini")
						addMultiMapValues(definition, readMultiMap(kUserHomeDirectory . "Setup\Definitions\" . step.Step . " Step.ini"))

					for language, ignore in availableLanguages()
						if inList(["EN", getLanguage()], language)
							for ignore, rootDirectory in [kResourcesDirectory . "Setup\Translations\", kUserHomeDirectory . "Setup\Translations\"]
...
loadDefinition() {
	...
	if FileExist(kUserHomeDirectory . "Setup\Simulator Setup.ini") {
	...
	if FileExist(kUserHomeDirectory . "Setup\Simulator Setup.ini")
		addMultiMapValues(definition, readMultiMap(kUserHomeDirectory . "Setup\Simulator Setup.ini"))
...
#Include "Libraries\SimpleSetupWizard.ahk"
