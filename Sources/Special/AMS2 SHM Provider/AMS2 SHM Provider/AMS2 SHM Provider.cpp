...
int main(int argc, char* argv[]) {
	...
	if (writeTelemetry) {
		...
		*/
		...
		printf("Running=%f\n", (long)normalize(localCopy->mParticipantInfo[localCopy->mViewedParticipantIndex].mCurrentLapDistance / localCopy->mTrackLength));
		...
		if (fileHandle == NULL) {
			...
			for (int i = 1; i <= localCopy->mNumParticipants; ++i) {
				...
				if (i == localCopy->mViewedParticipantIndex + 1)
					printf("Car.%d.Lap.Running.Time=%ld\n", i, (long)localCopy->mCurrentTime * 1000);
