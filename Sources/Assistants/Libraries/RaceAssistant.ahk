...
class RaceAssistant extends ConfigurationItem {
	...
	getKnowledge(type, options := false) {
		...
		local mixedCompounds, tyreSets, index, tyre, axle, unit
	...
	timeRecognized(words) {
		this.getSpeaker().speakPhrase("Time", {time: FormatTime(A_Now, "Time")})
	}
	...
	createSessionInfo(simulator, car, track, lapNumber, valid, data) {
		...
		if knowledgeBase {
			...
			setMultiMapValue(sessionInfo, "Stint", "Laps", (lapNumber > 0) ? (lapNumber - this.BaseLap + 1) : 0)
	...
	saveSessionSettings() {
		...
		if this.hasEnoughData(false) {
			...
			if ((loadSettings = "SettingsDatabase") || (loadSettings = "SetupDatabase")) {
				...
				settingsDB.setSettingValue(simulator, car, track, "*", "*", "Session Settings", "Fuel.Amount"
										 , Max(Round(knowledgeBase.getValue("Session.Settings.Fuel.Max"))
											 , settingsDB.getSettingValue(simulator, car, track, "*", "*", "Session Settings", "Fuel.Amount", 0)))
...
class GridRaceAssistant extends RaceAssistant {
	...
	updateSessionValues(values) {
		...
		if values.HasProp("TrackLength")
			this.initializeNormalizedRunnings()
	...
	trackGapToAheadRecognized(words) {
		...
		else if ((knowledgeBase.getValue("Car." . car . ".Laps", 0) > 0) && (delta != 0)) {
	...
	standingsGapToAheadRecognized(words) {
		...
		local car := knowledgeBase.getValue("Driver.Car", kUndefined)
		local speaker := this.getSpeaker()
		local delta, lap, laps, inPit, speaker
		...
		if ((car == kUndefined) || (car == 0))
			...
			try {
				...
				if ((car = kUndefined) || (laps == 0) || (delta == 0)) {
					...
					if ((laps > lap) && (Abs(delta) > (knowledgeBase.getValue("Lap." . lap . ".Time") / 1000)))
	...
	standingsGapToBehindRecognized(words) {
		...
		local speaker := this.getSpeaker()
		local car := knowledgeBase.getValue("Driver.Car", kUndefined)
		local delta, speaker, driver, inPit, lap, laps, lapped
	...
	gapToFocusRecognized(words) {
		...
		local speaker := this.getSpeaker()
		local car := knowledgeBase.getValue("Driver.Car", kUndefined)
		local number, delta, inPit, lapped, lap, laps
	...
	initializeNormalizedRunnings(trackLength := this.TrackLength) {
		local normalizedRunnings := []

		normalizedRunnings.Capacity := Round(trackLength / 5)

		loop (Round(trackLength / 5))
			normalizedRunnings.Push(false)

		this.iNormalizedRunnings := normalizedRunnings
		this.iHasNormalizedRunnings := false
	}

	hasNormalizedRunnings() {
		return this.iHasNormalizedRunnings
	}

	setNormalizedRunning(running, normalizedRunning) {
		local normalizedRunnings := this.iNormalizedRunnings
		local index := Round(Min(running, 1) * normalizedRunnings.Length)
		local currentRunning

		if index {
			currentRunning := this.iNormalizedRunnings[index]
	...
	getNormalizedRunning(running, data := false, variability := 25) {
		...
		local normalizedRunnings := this.iNormalizedRunnings
		local normalizedRunning, index

		if (this.iHasNormalizedRunnings && car && running) {
			index := Round(Min(running, 1) * normalizedRunnings.Length)
	...
	setTimeIntoLap(car, running, time, lapTime := false, data := false) {
		if !lapTime
	...
	getTimeIntoLap(car, running, data := false, lapTime := false, variability := 25) {
		local normalizedRunning := this.getNormalizedRunning(running, data, variability)

		if !lapTime
			lapTime := this.getLapTime(car, data)

		if normalizedRunning
			return (lapTime * normalizedRunning)
		else
	...
	getDelta(car, data := false) {
		...
		if data {
			...
			if driver {
				...
				return (((carLap * driverTime) + this.getTimeIntoLap(driver, this.getRunning(car, data), data))
					  - ((driverLap * driverTime) + this.getTimeIntoLap(driver, this.getRunning(driver, data), data)))
			}
			else
				return false
		}
			...
			if driver {
				...
				carLap := knowledgeBase.getValue("Car." . car . ".Laps", knowledgeBase.getValue("Car." . car . ".Lap", 0))

				return (((carLap * driverTime) + this.getTimeIntoLap(driver, this.getRunning(car, data)))
					  - ((driverLap * driverTime) + this.getTimeIntoLap(driver, this.getRunning(driver, data))))
			}
			else
				return false
		}
	...
	prepareData(lapNumber, data, force := false) {
		...
		if (force || (sector != lastSector) || (lapNumber != lastLap)) {
			lastSector := sector
			lastLap := lapNumber
	...
	addLap(lapNumber, &data) {
		...
		if driver {
			if (getMultiMapValue(data, "Position Data", "Car." . driver . ".Laps", getMultiMapValue(data, "Position Data", "Car." . driver . ".Lap", false)) = lapNumber) {
		...
		if (driver && knowledgeBase)
			for ignore, type in ["Standings.Overall.Leader", "Standings.Overall.Ahead", "Standings.Overall.Behind"
				...
				if this.getNormalizedRunning(knowledgeBase.getValue("Position." . type . ".Distance", false)) {
					delta := this.getDelta(knowledgeBase.getValue("Position." . type . ".Car"))
...
computePositions(context, lapNumber) {
	...
	local driverLaps := (knowledgeBase.getValue("Car." . driverCar . ".Laps"
					   ...
					   + driverRunning)
	local driverClass := knowledgeBase.getValue("Car." . driverCar . ".Class", "Unknown")
	local driverLapTime := knowledgeBase.getValue("Car." . driverCar . ".Time")
	local position, prefix, carLaps, carRunning
		...
		if ((knowledgeBase.getValue("Car." . car . ".Class") = class) && (delta > 0))
			if ((knowledgeBase.getValue("Position.Standings.Class.Ahead.Car", kUndefined) == kUndefined)
			 || (knowledgeBase.getValue("Position.Standings.Class.Ahead.Delta") > delta)) {
				...
				knowledgeBase.setFact("Position.Standings.Class.Ahead.Distance", distance)
				knowledgeBase.setFact("Position.Standings.Class.Ahead.Delta", delta)
			}
	}
		...
		if ((knowledgeBase.getValue("Car." . car . ".Class") = class) && (delta < 0))
			if ((knowledgeBase.getValue("Position.Standings.Class.Behind.Car", kUndefined) == kUndefined)
			 || (knowledgeBase.getValue("Position.Standings.Class.Behind.Delta") < delta)) {
				...
				knowledgeBase.setFact("Position.Standings.Class.Behind.Distance", distance)
				knowledgeBase.setFact("Position.Standings.Class.Behind.Delta", delta)
			}
	}
		...
		if (knowledgeBase.getValue("Car." . car . ".Class") = class) {
			...
			if ((leaderCar == kUndefined)
			 || (knowledgeBase.getValue("Car." . car . ".Position") < knowledgeBase.getValue("Car." . leaderCar . ".Position"))) {
				knowledgeBase.setFact("Position.Standings.Class.Leader.Car", car)
				knowledgeBase.setFact("Position.Standings.Class.Leader.Distance", distance)
				knowledgeBase.setFact("Position.Standings.Class.Leader.Delta", delta)
			}
		}
	}

	updateStandingsOverallAhead(car, distance, delta) {
		if (delta > 0)
			if ((knowledgeBase.getValue("Position.Standings.Overall.Ahead.Car", kUndefined) == kUndefined)
			 || (knowledgeBase.getValue("Position.Standings.Overall.Ahead.Delta") > delta)) {
				knowledgeBase.setFact("Position.Standings.Overall.Ahead.Car", car)
				knowledgeBase.setFact("Position.Standings.Overall.Ahead.Distance", distance)
				knowledgeBase.setFact("Position.Standings.Overall.Ahead.Delta", delta)
			}
	}

	updateStandingsOverallBehind(car, distance, delta) {
		if (delta < 0)
			if ((knowledgeBase.getValue("Position.Standings.Overall.Behind.Car", kUndefined) == kUndefined)
			 || (knowledgeBase.getValue("Position.Standings.Overall.Behind.Delta") < delta)) {
				knowledgeBase.setFact("Position.Standings.Overall.Behind.Car", car)
				knowledgeBase.setFact("Position.Standings.Overall.Behind.Distance", distance)
				knowledgeBase.setFact("Position.Standings.Overall.Behind.Delta", delta)
			}
	}
	...
	updateStandings(class, car, distance, delta) {
		updateStandingsClassAhead(class, car, distance, delta)
		updateStandingsClassBehind(class, car, distance, delta)
		updateStandingsClassLeader(class, car, distance, delta)
		updateStandingsOverallAhead(car, distance, delta)
	...
	updateTrackAhead(car, distance, delta) {
		if (distance < 0)
			if ((knowledgeBase.getValue("Position.Track.Ahead.Car", kUndefined) == kUndefined)
			 || (knowledgeBase.getValue("Position.Track.Ahead.Distance") < distance)) {
				knowledgeBase.setFact("Position.Track.Ahead.Car", car)
				knowledgeBase.setFact("Position.Track.Ahead.Distance", distance)
				knowledgeBase.setFact("Position.Track.Ahead.Delta", delta)
			}
	}

	updateTrackBehind(car, distance, delta) {
		if (distance > 0)
			if ((knowledgeBase.getValue("Position.Track.Behind.Car", kUndefined) == kUndefined)
			 || (knowledgeBase.getValue("Position.Track.Behind.Distance") > distance)) {
				knowledgeBase.setFact("Position.Track.Behind.Car", car)
				knowledgeBase.setFact("Position.Track.Behind.Distance", distance)
				knowledgeBase.setFact("Position.Track.Behind.Delta", delta)
			}
	}

	updateTrack(car, distance, delta) {
		updateTrackAhead(car, distance, delta)
		updateTrackBehind(car, distance, delta)
	}
	...
	loop carCount {
		...
		if (A_Index == driverCar) {
			...
			carRunning := knowledgeBase.getValue("Car." . A_Index . ".Lap.Running", 0)
...
speakAssistant(context, message, force := false) {
	...
	if assistant.Speaker[force] {
		...
		else if InStr(message, translate(". ")) {
