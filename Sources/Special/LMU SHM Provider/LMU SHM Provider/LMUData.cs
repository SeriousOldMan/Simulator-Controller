/*
Based on work of: The Iron Wolf (vleonavicius@hotmail.com)
...
namespace LMUSHMProvider
{
  ...
  // ULONGLONG     ->    Int64
  public class LeMansUltimateConstants
  {
    ...
    public const string LeMansUltimate_PROCESS_NAME = "LeMansUltimate";
    ...
    public enum LMUYellowFlagState
    {
      Invalid = -1,
      NoFlag = 0,
      Pending = 1,
    ...
    public enum LMURearFlapLegalStatus {
      Disallowed = 0,
    ...
    public enum LMUIgnitionStarterStatus {
      Off = 0,
      Ignition = 1,
    ...
    public enum LMUSafetyCarInstruction {
      NoChange = 0,
      GoActive = 1,
      HeadForPits = 2
    }
  ...
  namespace LeMansUltimateData
  {
    [StructLayout(LayoutKind.Sequential, Pack = 4)]
    public struct LMUVec3
    {
      public double x, y, z;
    }
    ...
    public struct LMUVehicleTelemetry
    {
      ...
      public LMUVec3 mPos;                  // world position in meters
      public LMUVec3 mLocalVel;             // velocity (meters/sec) in local vehicle coordinates
      public LMUVec3 mLocalAccel;           // acceleration (meters/sec^2) in local vehicle coordinates

      // Orientation and derivatives
      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 3)]
      public LMUVec3[] mOri;               // rows of orientation matrix (use TelemQuat conversions if desired), also converts local
                                           // vehicle vectors into world X, Y, or Z using dot product of rows 0, 1, or 2 respectively
      public LMUVec3 mLocalRot;             // rotation (radians/sec) in local vehicle coordinates
      public LMUVec3 mLocalRotAccel;        // rotational acceleration (radians/sec^2) in local vehicle coordinates
      ...
      public LMUVec3 mLastImpactPos;        // location of last impact
      ...
      public LMUWheel[] mWheels;                      // wheel info (front left, front right, rear left, rear right)
    }
    ...
    public struct LMUScoringInfo
    {
      ...
      public LMUVec3 mWind;                   // wind speed
    ...
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUPhysicsOptions
    {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    public enum LMUTrackRulesCommand
    {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    [StructLayout(LayoutKind.Sequential, Pack = 4)]
    public struct LMUTrackRulesAction
    {
      // input only
      public LMUTrackRulesCommand mCommand;        // recommended action
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUTrackRulesParticipant
    {
      // input only
      ...
      public LMUTrackRulesColumn mColumnAssignment;// which column (line/lane) that participant is supposed to be in
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    public enum LMUTrackRulesStage
    {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUTrackRules
    {
      // input only
      public double mCurrentET;                    // current time
      public LMUTrackRulesStage mStage;            // current stage
      public LMUTrackRulesColumn mPoleColumn;      // column assignment where pole position seems to be located
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUWeatherControlInfo
    {
    ...
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUMappedBufferVersionBlock
    {
    ...
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUMappedBufferVersionBlockWithSize
    {
      public uint mVersionUpdateBegin;          // Incremented right before buffer is written to.
      public uint mVersionUpdateEnd;            // Incremented after buffer write is done.
    ...
    public struct LMUTelemetry
    {
      ...
      public int mNumVehicles;                  // current number of vehicles
      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_MAPPED_VEHICLES)]
      public LMUVehicleTelemetry[] mVehicles;
    }


    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUScoring
    {
      public uint mVersionUpdateBegin;          // Incremented right before buffer is written to.
      public uint mVersionUpdateEnd;            // Incremented after buffer write is done.
      ...
      public LMUScoringInfo mScoringInfo;

      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_MAPPED_VEHICLES)]
      public LMUVehicleScoring[] mVehicles;
    }


    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMURules
    {
      public uint mVersionUpdateBegin;          // Incremented right before buffer is written to.
      public uint mVersionUpdateEnd;            // Incremented after buffer write is done.
      ...
      public LMUTrackRules mTrackRules;

      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_MAPPED_VEHICLES)]
      public LMUTrackRulesAction[] mActions;

      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_MAPPED_VEHICLES)]
      public LMUTrackRulesParticipant[] mParticipants;
    }


    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUForceFeedback
    {
      public uint mVersionUpdateBegin;          // Incremented right before buffer is written to.
      public uint mVersionUpdateEnd;            // Incremented after buffer write is done.
    ...
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUGraphicsInfo
    {
      public LMUVec3 mCamPos;              // camera position

      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 3)]
      public LMUVec3[] mCamOri;           // rows of orientation matrix (use TelemQuat conversions if desired), also converts local
    ...
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUGraphics
    {
      public uint mVersionUpdateBegin;          // Incremented right before buffer is written to.
      public uint mVersionUpdateEnd;            // Incremented after buffer write is done.

      public LMUGraphicsInfo mGraphicsInfo;
    }


    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUPitInfo
    {
      public uint mVersionUpdateBegin;          // Incremented right before buffer is written to.
      public uint mVersionUpdateEnd;            // Incremented after buffer write is done.

      public LMUPitMenu mPitMenu;
    }
    ...
    struct LMUWeather
    {
      ...
      public LMUWeatherControlInfo mWeatherInfo;
    }


    [StructLayout(LayoutKind.Sequential, Pack = 4)]
    public struct LMUTrackedDamage
    {
    ...
    [StructLayout(LayoutKind.Sequential, Pack = 4)]
    public struct LMUVehScoringCapture
    {
    ...
    [StructLayout(LayoutKind.Sequential, Pack = 4)]
    public struct LMUSessionTransitionCapture
    {
      ...
      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_MAPPED_VEHICLES)]
      public LMUVehScoringCapture[] mScoringVehicles;
    }


    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUExtended
    {
      public uint mVersionUpdateBegin;          // Incremented right before buffer is written to.
      public uint mVersionUpdateEnd;            // Incremented after buffer write is done.
      ...
      public LMUPhysicsOptions mPhysics;

      // Damage tracking for each vehicle (indexed by mID % LMUMappedBufferHeader::MAX_MAPPED_IDS):
      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_MAPPED_IDS)]
      public LMUTrackedDamage[] mTrackedDamages;
      ...
      public LMUSessionTransitionCapture mSessionTransitionCapture;  // Contains partial internals capture at session transition time.
      ...
      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_STATUS_MSG_LEN)]
      ...
      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_RULES_INSTRUCTION_MSG_LEN)]
    ...
    public struct LMUHWControl
    {
      ...
      [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = LeMansUltimateConstants.MAX_HWCONTROL_NAME_LEN)]
    ...
    [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 4)]
    public struct LMUWeatherControl
    {
      public uint mVersionUpdateBegin;          // Incremented right before buffer is written to.
      public uint mVersionUpdateEnd;            // Incremented after buffer write is done.
