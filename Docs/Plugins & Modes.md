...
The "Race Strategist" plugin handles the interaction of the currently active simulation as represented by the plugins "ACC", "RF2", "R3E", and so on, and Cato, the Virtual Race Strategist. If one of these simulation is started, the "Race Strategist" plugin will be automatically activated, and will start Cato in the background according to the configuration arguments described below. The following configuration parameters allow you to customize Cato to your preferences:
	...
	raceStrategist: *initialState* *onOffFunction*; raceStrategistName: *name*; raceStrategistLogo: true or false; raceStrategistLanguage: DE | EN | ...;
...
For Cato to be generally available, you must supply an argument for the *raceStrategistName* parameter.You can define a function on your hardware controller with the parameter *raceStrategist*, to enable or disable the Virtual Race Strategist dynamically. *initialState* must be either "On" or "Off" and for *onOffFunction* unary and binary functions are supported. The function will be bound to a plugin action.
