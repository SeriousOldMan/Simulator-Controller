...
| Function | Arguments                      | Description |
|----------|--------------------------------|-------------|
| Read     | simulator, [Optional] topics, [Optional] car, [Optional] track | Returns the data provided by the *simulator* for the given *car* and *track* combination for the specified *topics* (see below) incl. all post processing for tyre compound names, track names, etc. If *car* and *track* are not provided, the result will be the same, but processing will be slower, because two passes are required. The data is returned as string coded in "INI" format as [described in the Assistants documentation](https://github.com/SeriousOldMan/Simulator-Controller/wiki/AI-Race-Engineer#telemetry-integration).<br><br>The argument for *topics*, if supplied, must be a string containing a comma separated list of "All", "Session", "Standings" or "Pitstop" where "All" (which is also the default) stands for all of them. For performance aspects, only request the data you really need, because especially requesting the pitstop settings can take some time in several simulators.<br><br>Example: Simulator.Read("rFactor 2", "Session, Standings") |
...
| Data          | This property gives you access to the internal data represenation of the current session state (as returned by *Simulator.ReadSession*. It also contains the standings information as returned by *Simulator.ReadStandings*. It is a string in "INI" format as [described in the Assistants documentation](https://github.com/SeriousOldMan/Simulator-Controller/wiki/AI-Race-Engineer#telemetry-integration). Using this property is almost identical to calling the *Read* function of the "Simulator" module, but since this information is processed asynchronously, it may be a few seconds *old*. |
...
![](https://github.com/SeriousOldMan/Simulator-Controller/blob/main/Docs/Images/Simulator%20Module%20Example%201.jpg)
	...
	local function currentBrakeBalance()
		local ok, ini = pcall(Simulator.ReadSession, "Le Mans Ultimate")
