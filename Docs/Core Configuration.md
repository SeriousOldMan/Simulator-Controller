...
|      | Keyboard Hook  | False    | If set and *True*, a special keyboard hook is installed which may help when using a key on the keyboard should be used as Push-to-Talk button, but the simulator consumes all keyboard presses. But be careful, the technique used here is also used by key loggers and your AntiVirus software may have a problem with that. |
