...
| Category | Setting | Default | Description |
|----------|---------|---------|-------------|
| Voice     | Activation Recognizer  | Server    | Let's you choose the speech recognizer engine for the activation command used by the voice recognition. Allowed values here are: "Server", "Desktop", "Azure\|tokenIssuerEndpoint\|subscriptionKey" (with *tokenIssuerEndpoint* and *subscriptionKey* substituted by their corresponding values) and "Google\|apiKey" (with *apiKey* substituted by its corresponding value). |
...
|      | Interruptable | false     | If *true*, all current speeches will be interrupted, if Push-to-Talk is pressed. |
