...
namespace LMUSHMSpotter
{
    public class MappedBuffer<MappedBufferT>
    {
        ...
        readonly int LMU_BUFFER_VERSION_BLOCK_SIZE_BYTES = Marshal.SizeOf(typeof(LMUMappedBufferVersionBlock));
        readonly int LMU_BUFFER_VERSION_BLOCK_WITH_SIZE_SIZE_BYTES = Marshal.SizeOf(typeof(LMUMappedBufferVersionBlockWithSize));
        ...
        public bool GetMappedData(ref MappedBufferT mappedData)
        {
            ...
            using (var sharedMemoryStreamView = this.memoryMappedFile.CreateViewStream())
            {
                ...
                var versionHeaderWithSize = new LMUMappedBufferVersionBlockWithSize();
                var versionHeader = new LMUMappedBufferVersionBlock();
                ...
                for (retry = 0; retry < MappedBuffer<MappedBufferT>.NUM_MAX_RETRIEES; ++retry)
                {
                    ...
                    if (this.partial)
                    {
                        this.GetHeaderBlock<LMUMappedBufferVersionBlockWithSize>(sharedMemoryStream, this.LMU_BUFFER_VERSION_BLOCK_WITH_SIZE_SIZE_BYTES, ref versionHeaderWithSize);
                    ...
                    {
                        this.GetHeaderBlock<LMUMappedBufferVersionBlock>(sharedMemoryStream, this.LMU_BUFFER_VERSION_BLOCK_SIZE_BYTES, ref versionHeader);
                    ...
                    versionHeader = (LMUMappedBufferVersionBlock)Marshal.PtrToStructure(handleVersionBlock.AddrOfPinnedObject(), typeof(LMUMappedBufferVersionBlock));
