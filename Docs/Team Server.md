...
Please note, that you can copy the Server URI and the tokens to the clipboard using the small buttons on the right side of the fields and you can change your password by clicking on the small pencil beside the password field.
...
Data tokens can also be shared with your team mates. You do not have to create a team in this case. Simply send the token to everyone who will take part in the data replication partnership. See the documentation of the ["Session Database" tool](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Virtual-Race-Engineer#managing-the-session-database) for more information. The telemetry data shared using the central Team Server in this way will be used in the "Strategy Workbench", for example.
