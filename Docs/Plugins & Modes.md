...
The "Race Engineer" plugin handles the interaction of the currently active simulation as represented by the plugins "ACC", "RF2", "R3E", and so on, and Jona, the Virtual Race Engineer. If one of these simulation is started, the "Race Engineer" plugin will be automatically activated, and will start Jona in the background according to the configuration arguments described below. The following configuration parameters allow you to customize Jona to your preferences:
	...
	raceEngineerOpenSettings: *settingsFunction*; raceEngineerImportSetup: *importFunction*;
	raceEngineerOpenSetups: *setupsFunction*;
	raceEngineerLanguage: DE | EN | ...;
...
The "Race Strategist" plugin handles the interaction of the currently active simulation as represented by the plugins "ACC", "RF2", "R3E", and so on, and Cato, the Virtual Race Strategist. If one of these simulation is started, the "Race Strategist" plugin will be automatically activated, and will start Cato in the background according to the configuration arguments described below. The following configuration parameters allow you to customize Cato to your preferences:
	...
	raceStrategist: *initialState* *onOffFunction*; raceStrategistName: *name*; raceStrategistLogo: true or false; 
	raceStrategistOpenSettings: *settingsFunction*; raceStrategistOpenSetups: *setupsFunction*;
	raceStrategistLanguage: DE | EN | ...;
	raceStrategistSpeaker: false, true or *Microsoft Speech Generation Language*;
	raceStrategistListener: false, true or *Microsoft Speech Recognition Language*
	
For Cato to be generally available, you must supply an argument for the *raceStrategistName* parameter.You can define a function on your hardware controller with the parameter *raceStrategist*, to enable or disable the Virtual Race Strategist dynamically. *initialState* must be either "On" or "Off" and for *onOffFunction* unary and binary functions are supported. The function will be bound to a plugin action. Additionally, the parameter *raceStrategistOpenSettings* allows you to bind a plugin action to your hardware controller, which opens the [settings dialog](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Virtual-Race-Strategist#race-settings), which you will use before a race to give Cato the necessary information about your car setup and strategy options. And with *raceStrategistOpenSetups* you can open the [setup database query tool](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Virtual-Race-Engineer#querying-the-setup-database). If a simulation is currently running, most of the query arguments will already be prefilled. 
...
First, you need to define, how to open the Pitstop MFD in *Automobilista 2*. "I" is the default value for *openPitstopMFD*, which is **not** the standard binding of *Automobilista 2*. You need to change these bindings in *Automobilista 2*, since the standard bindings are controller buttons, for which unfortunately no events can be generated by software.
