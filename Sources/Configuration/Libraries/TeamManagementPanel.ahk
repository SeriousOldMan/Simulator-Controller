...
class TeamManagementPanel extends ConfiguratorPanel {
	iConnector := false
	iToken := false
	iInitialized := false
	...
	saveToConfiguration(configuration := false) {
	...
	connect(message := true, reconnect := false) {
		...
		if ((Trim(this.Control["teamServerURLEdit"].Text) != "") && (Trim(this.Control["teamServerNameEdit"].Text) != "")) {
			try {
				...
				this.iKeepAliveTask := PeriodicTask(ObjBindMethod(connector, "KeepAlive", connection), 120000, kLowPriority)

				this.iKeepAliveTask.start()
