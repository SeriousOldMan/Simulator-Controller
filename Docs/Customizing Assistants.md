...
Please be aware that you are talking to an AI based on GPT technology when using the *Conversation* booster. And AIs can lie or can give wrong answers. More details on that can be found below in the [How it works section](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Customizing-Assistants#how-it-works). To indicate that the upcoming answer will be generated by an LLM using the the *Conversation* booster, a short sound is played before the answer is given. As with the *Push-to-Talk* sound, you can route this acknowledge sound to a different device in a streaming setup, if required.
