...
class TeamDashboard extends ConfigurationItem {
	...
	createGui(configuration) {
		...
		Gui %window%:Add, DropDownList, x220 yp-2 w180 AltSubmit Choose1 +0x200 vanalysisMenuDropDown, % values2String("|", map(["Analysis", "---------------------------------------------", "Initialize from Setup Database...", "Initialize from Telemetry...", "Initialize from Simulation..."], "translate")*)
		
		Gui %window%:Add, DropDownList, x590 yp w180 AltSubmit Choose1 +0x200 vsimulationMenuDropDown, % values2String("|", map(["Simulation", "---------------------------------------------", "Run Simulation", "---------------------------------------------", "Use as Strategy..."], "translate")*)
		
		Gui %window%:Add, DropDownList, x405 yp w180 AltSubmit Choose1 +0x200 vstrategyMenuDropDown, % values2String("|", map(["Strategy", "---------------------------------------------", "Run Simulation", "---------------------------------------------", "Use as Strategy..."], "translate")*)
		
		Gui %window%:Add, DropDownList, x775 yp w180 AltSubmit Choose1 +0x200 vpitstopMenuDropDown, % values2String("|", map(["Pitstop", "---------------------------------------------", "Instruct Engineer..."], "translate")*)
		...
		Gui %window%:Add, ListView, x24 ys+34 w577 h170 -Multi -LV0x10 AltSubmit NoSort NoSortHdr HWNDlistHandle, % values2String("|", map(["#", "Driver", "Weather", "Compound", "Laps", "Pos. (Start)", "Pos. (End)", "Avg. Laptime", "Consumption", "Accidents", "Speed", "Consistency", "Car Control"], "translate")*)
		...
		Gui %window%:Add, Button, x400 ys w140 h50 gplanPitstop, % translate("Instruct`nEngineer...")
	...
	updateStint(stint) {
		...
		LV_Modify(stint.Row, "", stint.Nr, stint.DriverFullName, stint.Weather, stint.Compound, stint.Laps.Length()
							   ...
							   , stint.Speed, stint.Consistency, stint.CarControl)
	}
