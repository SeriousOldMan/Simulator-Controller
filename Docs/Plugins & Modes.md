...
With *trackAutomation* you can supply a function to enable or disable [position dependend actions](*) for your current session. The *onOffFunction* may be ommited, if you only want to enable or disable position dependend actions for all your sessions. The also optional *initialState* must be either "On" or "Off" (default is "Off") and for *onOffFunction* unary and binary functions are supported. The function will be bound to a plugin action.
