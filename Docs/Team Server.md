...
A word about memory consumption: Although a lot of effort has been put into optimizing the memory consumption of the overall solution, the Race Assistants still consume a lot of memory, which is especially important for endurance races. You can expect around 1GB of memory usage for a 1-hour stint, so take that into account for very long endurance races. A 32GB computer may be required when participating in 24-hour races, for example. Since this depends heavily on the settings, it may be possible to use a 16GB setup, though. I therefore recommend running a local test (i.e. Team Server on localost) with up to 3 stints and check the memory consumption of "Race Engineer.exe" and "Race Strategist.exe" afterwards. And I also recommend to check your pagefile settings - on a 16GB system, a pagefile size of factor 1.5 might helpful.
