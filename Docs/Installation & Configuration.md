...
Choose the simulator with the topmost drop down menu, before you change one of the settings beneath. In the first group, you can choose how the [settings](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Virtual-Race-Engineer#race-engineer-settings) for a new session and how the cold tyre pressures reference values will be initialized, whereas in the second group you define, wether the settings and the updated cold tyre pressures will be saved by the end of the session. If you choose to save the settings, the best lap time and the average fuel consumption will be used to update the session settings maintained by the *Race Engineer Settings* tool, either at the default location in the *Simulator Controller\Config* folder in you user *Documents* folder or in the corresponding settings information in the setup database (depending on the origin of the settings as configured in the first group). 
