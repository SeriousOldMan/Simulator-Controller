...
#### Changes
  ...
  - Support for the new shared memory API of *Le Mans Ultimate* has been added.
	...
	- A few data points are still acquired using the REST/JSON API, for example information about any suspension damage. It is possible to diable this API using a core setting [core setting](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Core-Settings#simulator-settings), if it becomes necessary due to performance issues.
