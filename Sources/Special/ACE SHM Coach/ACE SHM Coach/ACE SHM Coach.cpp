	...
	const std::vector<double>& rightAccelerations)
{
	...
	if (leftAccelerations.size() > 0) {
		...
		return std::vector<SuspensionBottomOuts>();
}
...
bool collectTelemetry(string soundsDirectory, string audioDevice, bool calibrate) {
	...
	int completedLaps = gf->total_lap_count;
	...
	if (fabs(steerAngle) > 0.2 && pf->speedKmh > 60) {
		...
		CornerDynamics cd = CornerDynamics(pf->speedKmh, 0, gf->total_lap_count, phase);
	...
	if (time(NULL) > nextUpdate) {
		...
		if ((velocityX != 0) || (velocityY != 0) || (velocityZ != 0)) {
			...
			if (strcmp(triggerType, "Trigger") == 0) {
				...
				if (lastLap != gf->total_lap_count) {
					lastLap = gf->total_lap_count;
