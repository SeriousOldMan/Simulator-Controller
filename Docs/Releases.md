## 5.2.3.0-release 08/18/23 (planned)
  ...
  5. The session info widget in the "System Monitor" will now also be updated for all tem mebers in a team race, which have not driven their first stint. Same is true for the JSON state file generated by the "Interface" plugin.
  ...
  7. [Experts Only] A new [controller action function "callCustom"](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Installation-&-Configuration#actions) has been addded, which allow you to activate custom controller functions from external scripts, for example.
