...
void sendCoachMessage(const char* message) {
	HWND winHandle = FindWindowExA(0, 0, 0, "Race Coach.exe");

	if (winHandle == 0)
		FindWindowExA(0, 0, 0, "Race Coach.ahk");

	if (winHandle != 0) {
		char buffer[128];

		strcpy_s(buffer, 128, "Race Coach:");
		strcpy_s(buffer + strlen("Race Coach:"), 128 - strlen("Race Coach:"), message);
...
bool checkAccident(const SharedMemory* sharedData)
{
	...
	try
	{
		...
		sendCoachMessage(("internalError:" + std::string(ex.what())).c_str());
	}
	catch (const std::string& ex) {
		sendCoachMessage(("internalError:" + ex).c_str());
	}
	catch (...) {
