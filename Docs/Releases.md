...
## 6.2.1.0-release 02/28/25 (planned)
  ...
  5. The track mapper resilience against precision deficits has been increased for *iRacing*.
  6. Increased resilience against corrupted telemetry layout definitions. Still do not understand, why theey get corrupted in the first place.
  7. The "Strategy Workbench" now includes an "Auto Initialize" setting (default is True) which triggers automatic initialization of several settings from the session database and/or a strategy, which has been loaded manually.
