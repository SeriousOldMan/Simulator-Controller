## 5.6.0.0-release 02/23/24 (planned)
  ...
  7. There is a new tab "Pitstops (fixed)" available in the "Strategy Workbench", which allows you to fix the lap for one or more pitstops during strategy simulation. But there are some drawbacks, so read the [new documentation](*) carefully.
  8. The rules for strategy validation for the "Strategy Workbench" have been revised and extended. Please see the [revised documentation](https://github.com/SeriousOldMan/Simulator-Controller/wiki/Virtual-Race-Strategist#scenario-validation) for more information.
  9. The default for the automatic pressure loss correction is now *Off* in the "Race Center".
  10. Several new RSS Formula cars have been included in the car meta data DLC.
