...
*Important*: When using the voice test, it might necessary due to technical reasons to use the double-press for activation commands. If there is only *one* dialog partner configured, this will be activated for listen mode by default. In this situation, no activation command is necesssary.
