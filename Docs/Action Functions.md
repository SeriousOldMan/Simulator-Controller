...
Many functions below are taking arguments. The follow rules apply:

1. The characters ",", ";" and "|" are reserved and may not be used inside an argument.
2. Arguments may be surrounded by double quotes. This is optional. The following two calls are identical:

		foo("Bar")
		foo(Bar)

   But these quotes have a meaning and are preserved:
   
		execute("D:\My Scripts\BB Handler.script" 1 49.7)
3. Single quotes "'" may also be used and are always preserved:

		foo('"Bar" "Baz"')
		
   But this is more a theoratical case and of not much use.
...
| Function | Parameter(s) | Plugin | Description |
| ------ | ------ | ------ | ------ |
| ask    | assistant, question, [Optional] command | Driving Coach, Race Engineer, Race Strategist, Race Spotter | This function will send the given *question* to the specified *assistant* as if it has been given by voice. This function will have no effect, if the Assistant is not configured for listening. If the optional argument *command* is supplied and *true*, full command processing is carried out, otherwise the *question* is passed directly to the *Conversation* booster. |
...
| invoke | target, method, [Optional] argument, ... | System | Invokes an internal method. *target* may be either "Controller" (or "Simulator Controller") for a method of the single controller instance itself or the name of a registered plugin or a name of a mode in the format *plugin*.*mode*. *method* is the name of the method to invoke for this target. You can supply any number of arguments to the invocation call and the return value is returned to the caller, if possible. |
| property | target, property, [Optional] argument, ... | System | Accesses a named property of an internal object and returns its value, if the caller can accept return values. *target* may be either "Controller" (or "Simulator Controller") for a property of the single controller instance itself or the name of a registered plugin or a name of a mode in the format *plugin*.*mode*. *property* is the name of the property to fetch from this target. You can supply any number of arguments to the property accessor. |
